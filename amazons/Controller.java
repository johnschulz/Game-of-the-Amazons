...
final class Controller {
    ...
    /**
     * The undo funtion implemented.
     * @param move is a dumby matcher
     */
    ...
    /**
     * Makes the manual player into our AI.
     * @param move is the white or black indicator.
     */
    private void doAuto(Matcher move) {
        String attempt = move.group();
        String x = move.group();
        attempt = attempt.replaceAll("\\s+", "");
        attempt = attempt.toUpperCase();
        if (attempt.equals("AUTOWHITE")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
        } else if (attempt.equals("AUTOBLACK")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        } else {
            _reporter.reportError("Error: Bad command: " + x);
        }
    }

    /**
     * Makes the player manual.
     * @param move is the white or black indicator.
     */
    private void doManual(Matcher move) {
        String attempt = move.group();
        String x = move.group();
        attempt = attempt.replaceAll("\\s+", "");
        attempt = attempt.toUpperCase();
        if (attempt.equals("MANUALWHITE")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else if (attempt.equals("MANUALBLACK")) {
    ...
    /**
     * Makes the move for the turn's player.
     * @param move clearifies the move.
     */
    private void doMove(Matcher move) {
        Move attempt = Move.mv(move.group());
        if (attempt != null) {
            if (_board.isLegal(attempt)) {
                _board.makeMove(attempt);
                ...
                _reporter.reportError("Invalid move. "
                        + "Please try again.");
            }
        }
    }
